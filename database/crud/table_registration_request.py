...
def add_registration_request(telegram_id: int, from_name: str, from_username: str, roles: str):
	session = db.SessionLocal()
...
def get_registration_request(request_id: int):
	session = db.SessionLocal()
	query = session.query(models.RegistrationRequest)
	return query.filter(models.RegistrationRequest.request_id == request_id).one_or_none()
